...
namespace csms_backend.Models.Entities
{
        ...
        public IQueryable<Pallet> PaginatedPallets(string? searchTerm)
        {
            if (!string.IsNullOrWhiteSpace(searchTerm))
            {
                var query = _context
                    .Pallet.AsNoTracking()
                    .Where(p =>
                        p.PalletType.Contains(searchTerm) || p.PalletType.Contains(searchTerm)
                    )
                    .Include(p => p.Creator)
    ...
    public class PalletPositionQuery
    {
        ...
        public IQueryable<PalletPosition> PaginatedPalletPositions(string? searchTerm)
        {
            ...
            {
                var query = _context
                    .PalletPosition.AsNoTracking()
                    .Where(p =>
                        p.Wing.Contains(searchTerm)
        ...
        {
            return await _context
                .PalletPosition.Include(p => p.ColdStorage)
                .FirstOrDefaultAsync(p => p.Id == id);
        }
    }
    ...
    public class ColdStorageQuery
    {
        ...
        {
            return await _context.ColdStorage.AsNoTracking().FirstOrDefaultAsync(c => c.Id == id);
        }

        public async Task<ColdStorage?> PatchColdStorageById(int id)
        {
            return await _context.ColdStorage.FirstOrDefaultAsync(c => c.Id == id);
        }
    }
}
